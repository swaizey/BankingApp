...
class UI{
	static showAlert(message, className){
		...
		app.insertBefore(div, accDetails)
...
//Search account
	...
	accounNumber =  document.querySelector('.accounNumber');
		if(amount.value === ''|| accounNumber.value === ''){
			UI.showAlert('Please fill all input', 'danger')
		}else{
			UI.showAlert(`Transaction Successful`, 'success')
			Store.deposit(Number(accounNumber.value), Number(amount.value))
		}
		amount.value = ''
		accounNumber.value = ''
...
function createAccount(e){
	...
	if(amount.value === ''|| accounNumber.value === ''){
			...
			UI.showAlert(`Transaction Successful`, 'success')
			Store.withdraw(Number(accounNumber.value), Number(amount.value))}
			amount.value = ''
		accounNumber.value = ''	
	}
...
function getStatement(e){
	...
	if(e.target.className === 'btn-statement'){
		const accounNumber = document.querySelector('.accounNumber')
		if(accounNumber.value === ''){
			UI.showAlert('Please provid an account', 'danger')
			}else{Store.showStatement(Number(accounNumber.value))
				accounNumber.value = ''
			}
	}
